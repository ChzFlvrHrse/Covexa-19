...
class GetCovidCasesIntentHandler(AbstractRequestHandler):
  """Handler for the get blog intent"""
  def can_handle(self, handler_input):
    return ask_utils.is_intent_name("GetCovidCasesIntent")(handler_input)
  ...
  def handle(self, handler_input):
    ...
    if alexa_response.status_code in range (200, 203):
      ...
      if deathRate <= .01:
        ...
        speak_output = f'The number of covid cases in your state thus far is {cases}. The number of covid deaths in your state is {deaths}. That is a {percentage} death rate. Looks like you will fair pretty well, but don\'t be too sure! Remember, wear a mask and get vaccinated, or you\'ll be adding to that number!'
      elif deathRate > .03:
        speak_output = f'The current death rate in your state is {percentage}, and it is spiraling out of control! Hide yo kids. Hide yo wife. Wear yo mask. Get yo vaccine. It\'s coming for you.'
      ...
      percentage = "{:.0%}".format(deathRate)
      if deathRate <= .01:
        speak_output = f'If you want to see how your state is doing, remember to enable location permission the next time you use this skill. Until then, here are your chances in the United States. The number of covid cases in the US thus far is {cases}. The number of covid deaths in the US thus far is {deaths}. That is a {percentage} death rate. Looks like you are in great shape. But remember, wear a mask, or you could be next!'
      elif deathRate > .01 and deathRate <= .03:
        speak_output = f'If you want to see how your state is doing, remember to enable location permission the next time you use this skill. Until then, here are your chances in the United States. The number of covid cases in the US thus far is {cases}. The number of covid deaths in the US thus far is {deaths}. That is a {percentage} death rate. Looks like you will fair pretty well, but don\'t be too sure! Remember wear a mask and get vaccinated, or you\'ll be adding to that number!'
      elif deathRate > .03:
        speak_output = f'If you want to see how your state is doing, remember to enable location permission the next time you use this skill. Until then, here are your chances in the United States. The current death rate is {percentage}, and it is spiraling out of control! It\'s coming for you. Ruuuuuuuuuun!'
